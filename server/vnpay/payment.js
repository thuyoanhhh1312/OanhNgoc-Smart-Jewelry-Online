...
router.post("/create_payment_url", (req, res) => {
  ...
  const returnUrl = `${process.env.NGROK_URL}/api/payment/vnpay_return`;
  ...
  try {
    ...
    const [orderItems] = await db.execute(
      `SELECT oi.*, p.product_name, p.sku, pi.image_url 
       ...
       LEFT JOIN product_images pi ON p.product_id = pi.product_id AND pi.product_id = oi.product_id
       WHERE oi.order_id = ?`,
      [orderId]
    );
